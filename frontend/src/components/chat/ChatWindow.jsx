...
function ChatWindow({user_id, chat_object, messages, onMessageSend }) {
    ...
    return <Card fluid className="border position-relative vh-100">
        ...
        <Stack style={{paddingBottom:"15%"}} className="border h-100 mh-100 overflow-y-scroll gap-2 " >
                {messages && messages.sort((a, b) => a.timestamp - b.timestamp).map((message,index) => <ChatMessage user_id={user_id} key={index} message_key={message.message_key} contents={message.contents} sender={message.sender_id} />)}
        </Stack>
        </CardBody>
