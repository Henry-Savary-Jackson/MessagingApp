...
import { generate25519KeyExchangePair, DH, extractC25519KeyExchangePair, exportX25519KeyPair, extractC25519KeySignaturePair, extractC25519ExchangePublicKey, concatenateUIntArray, generate25519SignaturePair, signPreKey, exportX25519PublicKey, signed_prekey_lifetime_ms } from "./CryptoUtils"
...
export async function store_message(indexed_db, message) {
    let chat_id = message.messageHeader.chat_id
...
export async function store_chat(indexed_db, dr_session) {
    await indexed_db.put(chats_store_name, dr_session)
}
...
export async function get_messages(indexed_db, chat_id) {
    return (await get_chat(indexed_db, chat_id)).messages || []
}
...
export async function getIdentityDataFromDB(indexed_db) {
    ...
    return (await indexed_db.openCursor(identity_store_name)).value
}
...
export async function create_chat_object( chat_id, name, type){
    return {
