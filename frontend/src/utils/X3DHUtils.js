...
export async function handle_X3DH_message( identity, signed_prekey, chat_message) {
    ...
    let otp_crypto_key = !one_time_prekey || await getOneTimePrekeyWithPubKey( one_time_prekey)
    if (one_time_prekey) {

        let under_full = await delete_one_time_prekey( one_time_prekey)
        async function refill() {
            let new_public_keys = await refill_otp()
